...
export class AppleProviderAdapter implements StoreProviderAdapter {
  readonly provider = "app_store_connect" as const;
  ...
  async createSyncPlan(args: Parameters<StoreProviderAdapter["createSyncPlan"]>[0]): Promise<SyncPlan> {
    return {
      ...
      items: args.diffItems.map((item, index): SyncPlanItem => ({
